...
A good way to understand your shooting style and guide your future camera equipment buying decisions will be to explore how you use lens and discover your frequently used focal lengths.

In this post, we are going to explore focal length usage with the flexible exiftool and R. exiftool, which is built using Perl and is available on Windows, Mac OS X and Linux, is used for extracting required EXIF information of your photos. Meanwhile R, is used to perform exploratory data analysis and visualization. R is also available on Windows, MAC OS X and Linux.
...
3. Various additional R packages, which will be described later.  
...
1\. We shall invoke the exiftool from R using the `system2` command. exiftool is capable of extracting a wide range of metadata from photos. We shall only extract common EXIF information and some additional EXIF tags for our purpose in addition to demonstrating the tools usage. For further details, please visit the [documentation](http://www.sno.phy.queensu.ca/~phil/exiftool/exiftool_pod.html)  
2\. We will be stating the focal length in terms of 35mm film format (or full frame format) allowing us to view our usage across several camera devices (for example smartphones, DSLRs, Mirrorless Cameras, Point-and-Shoot, etc) - which is very common nowadays - with a standard benchmark.  
3\. The extracted information will be saved in a tabluar format by exiftool to a CSV file named photodata.csv" for easy reading into R.  
4\. Replace the filepath below `d:/docs/photos` accordingly based on your system OS and location of the photos.  
5\. Please note that the code below performs a recursive search (switch `-r`) on subdirectories.  
...
After further exploration, data cleaning is deemed necessary. Please note that your situation might differ significantly.  

1\. In addition to file type, I also have alot of camera models in my data. Filtering to commonly used cameras is needed.  
...
2\. Not all cameras populate the `FocalLengthIn35mmFormat` tag, so we have to manually compute using the 35mm equivalent focal length variables. A new variable `FocalLengthUsed` is created: If a record has `FocalLengthIn35mmFormat` available, we shall use the value, otherwise `FocalLength * ScaleFactor35efl` is calculated and used.  
3\. __Please note:__ As per the [exiftool documentation](http://www.sno.phy.queensu.ca/~phil/exiftool/TagNames/Composite.html), any resized images will lead to inaccurate values reported in `ScaleFactor35efl`, thus `FocalLengthUsed` might have inaccuracies included.  
...
It can be seen that I shot mostly with normal lenses and tried out wide, portrait and telephoto ranges in 2012. Over these 3 years, my style progressively shifted to shooting with wide and normal lenes.

The 34mm result in 2014 is sort of surprising as it should be attributed to the 35mm lens that I use. It could be due to the resizing of images as discussed earlier.

It is worth to point out again that we are using the 35mm Format to standardize results. Therefore if you are shooting a 35mm lens with a crop sensor camera like Nikon D7000, you will get more counts on the 52mm focal length.

I hope you like this post, which is just one example demonstrating that there are a lot of data sets in our everyday lives that can be explored, allowing us to understand more of ourselves and act on them.
